...
\subsection{Se você for o novo mantenedor principal}

Se você está assumindo a manutenção principal do site (por exemplo, porque o mantenedor anterior saiu do
projeto), o primeiro passo é criar um \emph{fork} do repositório na sua própria conta do GitHub.
Esse fork é uma cópia completa do projeto, com todo o histórico, e é o que garante continuidade,
administração e controle total mesmo que o repositório original deixe de existir.

Você pode fazer isso pelo terminal ou pelo site do GitHub. Escolha a opção que preferir.

\textbf{Opção A: pelo terminal (GitHub CLI)}

Instale o \texttt{gh} se ainda não tiver:
...
Depois, autentique-se com a sua conta do GitHub (só precisa fazer isso uma vez):

\begin{terminal}
gh auth login
\end{terminal}

Com o \texttt{gh} autenticado, o comando abaixo já cria o fork na sua conta, clona o fork para o seu
computador e configura o repositório original como remoto \texttt{upstream} (útil para trazer atualizações
dele no futuro):

\begin{terminal}
gh repo fork BiduRuiz/mathfoundq --clone=true --remote=true
cd mathfoundq
\end{terminal}

\textbf{Opção B: pelo site do GitHub}

\begin{enumerate}
  \item Abra a página do repositório original no GitHub.
  \item Clique no botão \textbf{Fork}, no canto superior direito da página.
  \item Confirme a criação do fork na sua própria conta.
\end{enumerate}

Depois, clone o fork para o seu computador. Troque \texttt{SEU\_USUARIO} pelo seu nome de usuário do GitHub:

\begin{terminal}
git clone https://github.com/SEU_USUARIO/mathfoundq mathfoundq
cd mathfoundq
\end{terminal}

Se quiser poder trazer atualizações do repositório original depois, configure-o também como remoto
\texttt{upstream} (a Opção A já faz isso automaticamente):

\begin{terminal}
git remote add upstream https://github.com/BiduRuiz/mathfoundq.git
\end{terminal}

A partir desse momento, o fork na sua conta passa a ser o repositório oficial, o que você deve administrar
e manter atualizado.
